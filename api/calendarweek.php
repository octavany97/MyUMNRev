	...
	if($nim != ""){
		...
		if($res->num_rows > 0){
			...
			while($rs = $res->fetch_assoc()) {
			   ...
			   /*
				SELECT k.kodeKelas AS kk, k.ruangKelas AS rk, k.hari AS h, e.tanggalMulai AS tgl, k.jamMulai AS jm, k.jamSelesai AS js, m.kodeMatkul AS km, m.namaMatkul AS nm, IFNULL((SELECT CONCAT(i.namaDepan, ' ', i.namaBelakang)  
                                   FROM InfoUser i, Users u, Kelas k  
                                   WHERE i.infoID = u.infoID 
                                   AND u.userID = k.dosenID 
                                   AND k.kelasID = ". $rs['ki'] . "), 'n/a') 'namaDosen' 
				FROM kelas k, matakuliah m, event e
				WHERE k.kelasID = " . $rs['ki'] . "
				AND k.matkulID = m.matkulID
				AND k.hari = DAYNAME(e.tanggalMulai)
				AND e.keterangan LIKE 'Masa Perkuliahan ".$sem."'
